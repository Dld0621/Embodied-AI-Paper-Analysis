const I18N = {
  en: {
    ...
    eyebrow: "Conference + rolling arXiv census", heroLineOne: "Research the field.", heroLineTwo: "Not the feed.",
    ...
    scope: "Research corpus", audited: "AUDITED", papers: "papers", researchRecords: "unique research records", conferencePapers: "conference papers", arxivPapers: "rolling three-year arXiv", arxivCandidates: "arXiv candidates audited", venues: "major venues", tracks: "research tracks", subfields: "level-2 subfields", specialties: "level-3 specialties", linked: "source linked",
    ...
    policyOneTitle: "Two explicit windows", policyOneBody: "A bounded conference census plus an execution-date-driven rolling three-year arXiv window.",
  ...
  zh: {
    ...
    policyOneTitle: "两个明确窗口", policyOneBody: "固定边界的顶会普查，加上由执行日期驱动的滚动三年 arXiv 窗口。",
...
function refreshSnapshotLabels() {
  ...
  const { start, end } = state.arxivCatalog.window;
  I18N.en.eyebrow = `Conference ${conferenceWindow} + rolling arXiv · updated ${end}`;
  I18N.zh.eyebrow = `顶会 ${conferenceWindow} + 滚动 arXiv · 更新于 ${end}`;
  I18N.en.arxivPapers = `arXiv · ${start} to ${end}`;
  I18N.zh.arxivPapers = `篇 arXiv · ${start} 至 ${end}`;
  I18N.en.policyOneBody = `Conference years ${conferenceWindow}; arXiv submissions from ${start} through ${end}.`;
  I18N.zh.policyOneBody = `顶会年份为 ${conferenceWindow}；arXiv 原始提交日期为 ${start} 至 ${end}。`;
}
